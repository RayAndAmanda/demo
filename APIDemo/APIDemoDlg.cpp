								 ...
								 int iDataLen, int iWidth, int iHeight, int iFrameType, int iTimeStamp, void* pUserData)
{
    ...
    if (tmp - g_pMainDlg->time4YUV > 1)
    {
        ...
        ::PostMessage(g_pMainDlg->GetSafeHwnd(), CALLBACKMSG, (WPARAM)pType, (LPARAM)pMsg);
		//add by ray
		/*WriteFile(hwFile, pDataArray, iDataLen, &rt, NULL);
...
BOOL CAPIDemoDlg::OnInitDialog()
{
	...
	//add by ray
	/*hwFile = CreateFile("a.yuv", GENERIC_WRITE, 0, NULL,
		CREATE_ALWAYS,
...
BOOL CAPIDemoDlg::OnCopyData(CWnd* pWnd, COPYDATASTRUCT* pCopyDataStruct)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	CopyDataCheck(m_hWnd, pCopyDataStruct);
	//char temp[1024];
	//memcpy(temp, (char *)pCopyDataStruct->lpData, pCopyDataStruct->cbData);
	//printf("getS200Messgae!...%x\n", pCopyDataStruct->dwData);
	//printf("getS200Messgae!  %s\n",(char *)pCopyDataStruct->lpData);
	
	return CDialog::OnCopyData(pWnd, pCopyDataStruct);
}


void CAPIDemoDlg::OnWindowPosChanging(WINDOWPOS* lpwndpos)
{
	//增加窗口隐藏功能
	CString UserInfo_path(".\\UserInfo.ini");
	int hideWindow = GetPrivateProfileInt("UserInfo", "hidewindow", 0, UserInfo_path);
